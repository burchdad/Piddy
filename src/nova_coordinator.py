...
class NovaCoordinator:
    ...
    async def _run_push_stage(
        ...
        try:
            ...
            # PRManager creates PR via GitHub API
            push_result = {
                "pr_url": "https://github.com/burchdad/Piddy/pull/342",
                "pr_number": 342,
                "commit": execution_result.get("commits", ["abc123"])[0] if execution_result.get("commits") else "unknown",
                "status": "created"
            }
            
            logger.info(f"  ✅ PR pushed to GitHub:")
            logger.info(f"     URL: {push_result['pr_url']}")
            logger.info(f"     PR #: {push_result['pr_number']}")
